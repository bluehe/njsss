...
class Pager {
    ...
    public function GenBasic() {
        ...
        /*
          ...
          if ($this->pageNo > 1) {
          $buffer .= "<li><a href='".$this->genURL($this->pageString,1)."'>{$index}</a></li><li><a href='".$this->genURL($this->pageString,$this->pageNo-1)."'>{$pre}</a></li>";
          }
          ...
          $buffer .= "<li><a href='".$this->genURL($this->pageString,$this->pageNo+1)."'>{$next}</a></li><li><a href='".$this->genURL($this->pageString,$this->pageCount)."'>{$end}</a></li>";
          }
          return $buffer . '</ul>';
         */

        $buffer .= "<div class=\"pages\">";
        if ($this->pageNo > 1) {
        ...
        if ($this->pageNo < $this->pageCount) {
            $buffer .= "<a href='" . $this->genURL($this->pageString, $this->pageNo + 1) . "'>{$next}</a><a href='" . $this->genURL($this->pageString, $this->pageCount) . "'>{$end}</a>";
        }
        return $buffer . "</div>";
    }
