...
class Utility {
    ...
    static public function ValidEmail($email, $strict = false) {
        ...
        if (preg_match($regexp, $email)) {
            ...
            if ($strict && !gethostbyname($domain) && !getmxrr($domain, $mxhosts)) {
                return false;
            }
            return true;
        }
    ...
    static public function GetHost($type = 'pre', $httpurl = '', $default = '') {
        //$httpurl = $_SERVER['HTTP_HOST'];
        $httpurl = strtolower(trim($httpurl));
        if (empty($httpurl)) {
            $httpurl = $default;
        }
        $regx1 = '/((https?\:\/\/)?([^\/\?#]+\.)?([^\/\?#-\.]+\.)(com\.cn|edu\.com\.cn|gov\.cn|org\.cn|net\.cn|com\.jp|co\.jp|com\.kr|com\.tw)(\:[0-9]+)?)(.+)/i';
        ...
        $port = ':80';  //端口
        $all = '';
        //1、域名；2、协议；3、前缀+.；4、域名+.；5、后缀；6、:+端口；7、？+参数
//        array(8) {
//            [0] => string(29) "http://m.taobao.com:8080?id=22&d=3"
//            [1] => string(19) "http://m.taobao.com"
//            [2] => string(7) "http://"
//            [3] => string(2) "m."
//            [4] => string(7) "taobao."
//            [5] => string(3) "com"
//            [6] => string(0) ":8080"
//            [7] => string(10) "?id=22&d=3"
//        }
        if (!preg_match($regx1, $httpurl, $matches))
            preg_match($regx2, $httpurl, $matches);
        if ($matches) {
            $pre = substr($matches[3], 0, -1) ? substr($matches[3], 0, -1) : 'www';
            $domain = $matches[4] . $matches[5] . $matches[6];
            $host = substr($matches[4], 0, -1);
            $all = $pre . '.' . $domain;
        }
        if ($type == 'domain') {
            return $domain;
    ...
    static public function GetPageNo($page = 'page') {
        $page_no = isset($_GET['page']) ? abs(intval($_GET['page'])) : 1;
    ...
    static public function ScanDir($d, $e = array(), $r = false) {
        ...
        foreach ($c as $o) {
            ...
            if (( empty($e) || in_array($eo, $e)) && is_file($p) && is_readable($p)) {
                $a[] = $p;
            } else if (is_dir($p) && is_readable($p) && true == $r) {
    ...
    static public function GetBanner($d, $e = array(), $r = false) {
        $c = scandir($d);
        $a = array();
        foreach ($c as $o) {
            if ($o == '.' || $o == '..')
    ...
    static function IsMobile($no) {
        return preg_match('/^1[3578][\d]{9}$/', $no);
    }

    static function GetNumCode($length = 5) {
        $pattern = '1234567890';
        for ($i = 0; $i < $length; $i++) {
    ...
    static public function GetParameter($name, $k = array()) {
        $one = isset($k) && !empty($k) ? true : false;
        $condition['name'] = $name;
        if ($one)
            $condition['k'] = $k;
        $res = DB::LimitQuery('parameter', array('condition' => $condition));
        $ret = array();
        foreach ($res as $r) {
            $ret[$r['k']] = $r['v'];
        }
