...
function dropmenu(obj) {
    ...
    $(obj.id + 'child').style.top = (parseInt($(obj.id + 'child').style.top) - document.body.scrollTop) + 'px';
    if (BROWSER.ie) {
...
function textareasize(obj, op) {
    if (!op) {
        ...
        if (obj.style.position == 'absolute') {
            ...
            obj.style.width = BROWSER.ie > 6 || !BROWSER.ie ? (op == '1' ? '80%' : '40%') : '600px';
...
function showanchor(obj) {
    ...
    for (var i = 0; i < navs.length; i++) {
        if (navs[i].id.substr(0, 4) == 'nav_' && navs[i].id != obj.id) {
            ...
            if ($(navs[i].id.substr(4) + '_tips'))
                $(navs[i].id.substr(4) + '_tips').style.display = 'none';
        }
    }
    obj.className = 'current';
    var currentAnchor = obj.id.substr(4);
    $(currentAnchor).style.display = '';
    if ($(currentAnchor + '_tips'))
        $(currentAnchor + '_tips').style.display = '';
