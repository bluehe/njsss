...
function loadcss(cssname) {
    if (!CSSLOADED[cssname]) {
        ...
        css.type = 'text/css';
        css.rel = 'stylesheet';
        css.href = '/themes/default/css/' + cssname + '.css';
...
function setMenuPosition(showid, menuid, pos) {
    ...
    if (direction == 0 || menuObj.scrolly) {
        if (BROWSER.ie && BROWSER.ie < 7) {
            if (direction == 0)
                mt += Math.max(document.documentElement.scrollTop, document.body.scrollTop);
        } else {
            if (menuObj.scrolly)
                mt -= Math.max(document.documentElement.scrollTop, document.body.scrollTop);
...
function adminloadpage(type, id, dir) {
    window.location.href = dir + '/admin/' + type + '?id=' + id;
}
