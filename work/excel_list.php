...
$objPHPExcel->getProperties()->setCreator("blue.he")
        ->setLastModifiedBy("blue.he");
...
foreach ($data as $key => $one) {
    ...
    if (isset($one['row_height'])) {
        $objPHPExcel->setActiveSheetIndex(0)->getRowDimension($key + 3)->setRowHeight($one['row_height']);
    } else {
...
//                $objDrawing->getShadow()->setDirection(50);
            ...
            if (array_key_exists('br', $title['style'])) {
//设置单元格“自动换行”属性
                $objPHPExcel->setActiveSheetIndex(0)->getStyle($a . ($key + 3))->getAlignment()->setWrapText(true);
            }
        }
